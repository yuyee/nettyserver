...
/**
 * Parser for JSON text. Please note that JSONParser is NOT thread-safe. From
...
public class JSONParser {
                ...
                switch (status) {
                    ...
                    case S_PASSED_PAIR_KEY:
                        ...
                        switch (token.type) {
                            ...
                            case Yytoken.TYPE_LEFT_SQUARE:
                                statusStack.removeFirst();
                                statusStack.addFirst(new Integer(
                                    S_IN_PAIR_VALUE));
                                status = S_IN_ARRAY;
                                statusStack.addFirst(new Integer(status));
                                if (!contentHandler.startArray()) {
     ...
     * 线程变量: 默认JSON解析器
     */
    private static final ThreadLocal<SoftReference<JSONParser>> localParser = new ThreadLocal<SoftReference<JSONParser>>();
     ...
     * @throws ParseException
     */
    public static Object defaultParse(String str) throws ParseException {
        SoftReference<JSONParser> parserRef = localParser.get();
        ...
        if (parser == null) {
            ...
            localParser.set(new SoftReference<JSONParser>(parser));
        }
    ...
    /**
     * 对JSON字符串进行解析. 如果出现解析错误不抛出异常,而是直接返回null
     * <p>
     * 保证线程安全
     * 
     * @param str
     *            待解析的字符串
     * @return JSON对象
     * @see #defaultParse(String)
     */
    public static Object defaultParseQuietly(String str) {
        try {
            return defaultParse(str);
        } catch (Exception e) {
            if (LOG.isLoggable(Level.FINEST)) {
                LOG.log(Level.FINEST, "parse str error. str=" + str, e);
            } else {
                LOG.log(Level.INFO, "parse str error. str=" + str + ",cause="
                    + e);
            }
