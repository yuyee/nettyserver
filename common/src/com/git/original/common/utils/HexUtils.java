...
public final class HexUtils {
	...
	public static String toHexString(ByteBuffer buf) {
		...
		if (buf.hasArray()) {
			// 直接利用底层数组
			str = toHexString(buf.array(), buf.arrayOffset() + buf.position(),
					buf.remaining(), null);
	...
	public static String toAbbrHexString(byte[] ba) {
		...
		for (int i = 0; i < l; i++) {
			...
			sb.append(digits[(0xF0 & ba[i]) >>> 4])
					.append(digits[0x0F & ba[i]]);
		}
	...
	public static byte[] parseHexString(CharSequence cs) {
		...
		if ((len & 0x1) == 0x1) {
			...
			if (c < 0 || c > 15) {
				throw new IllegalArgumentException(
						"illegal hex char position is: 0");
			}
		...
		for (; i < len; i += 2, j++) {
			...
			if (c1 < 0 || c1 > 15) {
				throw new IllegalArgumentException(
						"illegal hex char position is:" + i);
			}
			...
			if (c2 < 0 || c2 > 15) {
				throw new IllegalArgumentException(
						"illegal hex char position is:" + (i + 1));
			}
