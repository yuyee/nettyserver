...
public class DLeftBloomFilter {
     ...
     * 子表
     */
    private byte[][] subtables;

    /**
     * <ul>
     * 桶
     * <li>包含8个cell
     * <li>每个cell占用2bit，表示count
     * <li>每个桶占用8*2=16bit，即1个桶即等同short大小
     * </ul>
     */
     ...
     * hash数
     */
    public static final int HASHCOUNT = 4;

    /**
     * 一个count即2个bit能表示的最大数字
     */
    ...
    public int checkFingerPrint(int tableIndex, int bucketIndex,
        ...
        // 定位桶
        int bucketByteSize = this.bucketCapacity * bucketIndex;
        // 取此桶当前cell数
        int cellCount = subtables[tableIndex][bucketByteSize + fingerBitsSize];
        for (int i = 0; i <= cellCount; i++) {
            int begin = bucketByteSize + i * 3;
            // 取手印
            byte[] finger = new byte[3];
            System.arraycopy(subtables[tableIndex], begin, finger, 0, 3);
    ...
    public void add(String key) {
        ...
        if (info.isExist()) {
            ...
            // 暂不考虑并发带来的问题
            short count = buckets[tableIndex][bucketIndex];
            ...
            if (count < MAXCOUNT) {
                ...
                buckets[tableIndex][bucketIndex] = count;
            }
        } else {
            ...
            for (int i = 0; i < hashs.length; i++) {
                int bindex = hashs[i];
                // 定位桶
                int bucketByteSize = this.bucketCapacity * bindex;
                // 取此桶当前负载数
                int tempBucketCount = countBucketLoad(buckets[i][bindex],
                    subtables[i][bucketByteSize + fingerBitsSize]);
                // 负载比较
                if (tempBucketCount < bucketLoad) {
                    bucketLoad = tempBucketCount;
                    minCellIndex = subtables[i][bucketByteSize + fingerBitsSize];
            ...
            byte cellCount = subtables[tableIndex][this.bucketCapacity
            ...
            if (cellCount == 8) {
                ...
                // 定位后，把手印放进去
                int destIndex = this.bucketCapacity * minBucketIndex
                    + minCellIndex * 3;
                subtables[tableIndex][destIndex] = fingerPrint[3];
                subtables[tableIndex][destIndex + 1] = fingerPrint[2];
                subtables[tableIndex][destIndex + 2] = fingerPrint[1];
                // cellCount + 1
                subtables[tableIndex][this.bucketCapacity * minBucketIndex
    ...
    public void delete(String key) {
        ...
        if (count >= 1) {
            ...
            // 删除 定位
            int destIndex = bucketIndex * this.bucketCapacity + cellIndex * 3;
            System.arraycopy(DELBYTES, 0, subtables[tableIndex], destIndex, 3);
        }
    }// end del
